<script>
  ...
  const areaTargets = $derived(
    stackOrder.map((key, i) => {
      ...
      const lowerSumKeys = stackOrder.slice(0, i).flatMap((k) => getAreaKeys(k, step));
      const values = sumData.map((d) => {
        const lower = lowerSumKeys.reduce((s, k) => s + (d[k] ?? 0), 0);
        const upper = lower + sumKeys.reduce((s, k) => s + (d[k] ?? 0), 0);
  ...
  $effect(() => {
    ...
    // (protection ↔ plan/promotor/tenencia swap) so they don't accumulate.
...
<defs>
  ...
  >
    <rect width="6" height="6" fill="white" />
    <rect width="3" height="6" fill="var(--bw700)" />
  </pattern>
</defs>
...
{#each tweenedAreas as { key, values } (key)}
  <path
    ...
    stroke-width="1.5"
    data-key={key}
    fill={key === 'maybe' ? 'url(#maybe-stripes)' : undefined}
