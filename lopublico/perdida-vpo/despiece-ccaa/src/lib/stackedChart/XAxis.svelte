<script>
  ...
  // intermediates (every 10y mobile, 5y desktop). On desktop, round ticks too close
  // to the first year are dropped to avoid crowding.
  const visibleTicks = $derived.by(() => {
    ...
    if (!mobile) {
      const keyYears = new Set([start].filter((y) => ticks.has(y)));
      return [...ticks]
        .sort((a, b) => a - b)
        .filter(
          (t) =>
            keyYears.has(t) || ![...keyYears].some((k) => k !== t && Math.abs(t - k) < step * 0.6)
        );
    }
