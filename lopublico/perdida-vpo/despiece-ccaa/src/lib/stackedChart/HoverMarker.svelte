<script>
  const { hoveredYear, hoveredRow, scaleX, scaleY, stackOrder } = $props();
  ...
  const areaCircles = $derived.by(() => {
    ...
    return stackOrder
      .map((key, i) => {
        const lower = stackOrder.slice(0, i).reduce((s, k) => s + (hoveredRow[k] ?? 0), 0);
        const value = hoveredRow[key] ?? 0;
        return { key, value, upper: lower + value };
      })
