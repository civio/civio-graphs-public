<script>
  ...
  class IframeLoader {
    ...
    url = $derived(
      `https://graphs.civio.es/${__PROJECT_PATH__}/dist/iframes/${embedID}-${vizLang.value}-responsive.md`
    );
  ...
  // Load when URL changes (language or embedID changes)
  $effect(() => {
