<script>
  ...
  import { urlInfo, prefersReducedMotion } from '../../states/utils.svelte';
  ...
  const investigationLink = __INVESTIGATION_URL__;

  let showShareModal = $state(false);
  let shareModalButton = $state();

  const flyOpts = $derived(prefersReducedMotion.current ? { duration: 0 } : { y: 20 });

  // Generate unique IDs using chartID to avoid conflicts when multiple graphs are on the same page
  const shareModalId = $derived(`share-modal-${chartID}`);

  function toggleShareModal() {
    showShareModal = !showShareModal;
  }

  // Return focus to the toggle button so keyboard users keep their place
  function closeShareModal() {
    showShareModal = false;
    shareModalButton?.focus();
  }
  ...
  function focusFirstElement(node) {
    node.querySelector('button, a, input, [tabindex]')?.focus();
  }
</script>
  ...
  <div style="display: flex; gap: .5rem;align-items: center">
    ...
    {#if showShareModal}
      <div
        ...
        {@attach focusFirstElement}
        {@attach clickOutside({
          exclude: '.shareModalButton',
          onClickOutside: closeShareModal,
        })}
      >
        <ShareContainer texts={vizLang.texts} {chartID} />
      </div>
    {/if}
    <a
      href="https://civio.es"
    ...
    >
      <img src={urlInfo.isCivio ? logo : fullLogo} alt={vizLang.texts.logoAlt} class="civio-logo" />
    </a>
  </div>
</div>
