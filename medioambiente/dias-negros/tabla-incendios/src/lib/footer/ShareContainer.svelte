<script>
  ...
  import { prefersReducedMotion } from '../../states/utils.svelte.js';

  const { texts, chartID } = $props();
  ...
  // Cancel pending timers if the modal unmounts so they don't fire on a
  // destroyed component.
  $effect(() => () => {
  ...
  class IframeLoader {
    ...
    async load(signal) {
      try {
        ...
        // An abort means a newer request superseded this one: keep the state
        if (error.name !== 'AbortError') this.code = '';
      }
    }
  }
  ...
  // Load when the URL changes (language or chartID); aborting on cleanup
  // discards stale in-flight responses so they can't overwrite newer ones
  $effect(() => {
    const controller = new AbortController();
    iframeLoader.load(controller.signal);
  ...
  async function copyIframeCode() {
    if (!iframeLoader.code) {
      setTemporaryStatus(texts.loadingEmbed);
      return;
    }
    ...
    if (copied) {
      ...
      setTemporaryStatus(texts.codeCopied);
      copyResetTimeout = setTimeout(() => {
        displayCopyCode = false;
      }, 1500);
    } else {
      setTemporaryStatus(texts.embedError, 2000);
    }
  }
...
<ul style="list-style: none;" role="list">
  <li style="background-color: white;">
    <button
      ...
      style="z-index: 0"
      aria-label={displayCopyCode ? texts.codeCopied : texts.copyEmbed}
    >
      {#if displayCopyCode}
        <span
